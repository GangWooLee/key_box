...
| 스킬 | 설명 | 커버 파일 패턴 |
|------|------|---------------|
| `verify-example` | 예시 검증 | `app/models/**/*.rb` |
-->
...
| 디렉토리 | 파일 |
|----------|------|
| app/models | `user.rb`, `post.rb` |
| app/controllers | `users_controller.rb`, `posts_controller.rb` |
| app/services | `registration_service.rb` |
| test | `user_test.rb`, `post_test.rb` |
| (루트) | `Gemfile`, `.rubocop.yml` |
```
...
| 스킬 | 트리거 파일 (변경된 파일) | 액션 |
|------|--------------------------|------|
| verify-layered-arch | `users_controller.rb`, `registration_service.rb` | CHECK |
| verify-clean-code | `user.rb` | CHECK |
| (스킬 없음) | `Gemfile`, `.rubocop.yml` | UNCOVERED |
```
...
3. **커버되지 않은 새 패턴** — 변경된 파일을 읽고 스킬이 검사하지 않는 새로운 규칙, 설정, 패턴을 식별합니다. 확인 사항:
   - 새로운 모델 클래스, concern, 또는 서비스 객체
   - 새로운 라우팅이나 컨트롤러 액션
   - 새로운 Stimulus 컨트롤러나 Turbo Stream 패턴
4. **삭제된 파일의 잔여 참조** — 스킬의 Related Files에 있는 파일이 코드베이스에 더 이상 존재하지 않는 경우?
5. **변경된 값** — 스킬이 검사하는 특정 값(식별자, 설정 키, 클래스 이름)이 수정된 파일에서 변경되었는지?
...
| 스킬 | 갭 유형 | 상세 |
|------|---------|------|
| verify-layered-arch | 파일 누락 | `app/services/new_service.rb`가 Related Files에 없음 |
| verify-clean-code | 새 패턴 | 새 concern이 검사되지 않는 규칙을 사용 |
| verify-error-handling | 오래된 값 | rescue 패턴이 변경됨 |
```
...
**결정: 기존 스킬 UPDATE** (N개)
- `verify-layered-arch` — 누락된 파일 참조 2개 추가, 탐지 패턴 업데이트
- `verify-error-handling` — 새 설정 패턴에 대한 탐지 명령어 업데이트
...
|------|---------|
| ... 기존 항목 ... |
| `app/services/new_service.rb` | 비즈니스 로직을 포함한 새 서비스 객체 |
```
...
**검사:** 모델 콜백이 3개를 초과하지 않는지 확인.

```bash
grep -n "after_\|before_" app/models/*.rb
```
...
2. **사용자에게 스킬 이름 확인** — `AskUserQuestion`을 사용합니다:
   ...
   **이름 규칙:**
   - 이름은 반드시 `verify-`로 시작해야 합니다 (예: `verify-layered-arch`, `verify-error-handling`, `verify-clean-code`)
...
4. **연관 스킬 파일 업데이트** — 새 스킬 생성 후 반드시 아래 3개 파일을 업데이트합니다:
   ...
   **4c. `CLAUDE.md` 업데이트:**
   - `## Skills` 테이블에 새 스킬 행을 추가합니다 (있는 경우)
...
| `.claude/rules/**/*.md` | 프로젝트 규칙 (19개: backend 6, frontend 5, common 6, testing 2) |
...
1. **Lock 파일 및 생성된 파일** — `Gemfile.lock`, `yarn.lock`, `db/schema.rb`, 자동 생성된 마이그레이션 파일, 빌드 출력물은 스킬 커버리지가 불필요
2. **벤더/캐시 디렉토리** — `vendor/bundle/`, `node_modules/`, `public/assets/`, `tmp/`, `log/` 내의 파일은 외부 또는 생성된 코드
3. **일회성 설정 변경** — `Gemfile`의 버전 범프, `.rubocop.yml`의 사소한 변경은 새 스킬이 불필요
4. **문서 파일** — `README.md`, `CHANGELOG.md`, `docs/**/*.md` 등은 검증이 필요한 코드 패턴이 아님 (단, `.claude/rules/**/*.md`는 규칙 파일이므로 면제 아님)
5. **테스트 픽스처 파일** — `test/fixtures/` 디렉토리의 파일은 프로덕션 코드가 아님
6. **영향받지 않은 스킬** — UNAFFECTED로 표시된 스킬은 검토 불필요
7. **CLAUDE.md 자체** — CLAUDE.md의 변경은 문서 업데이트이며, 검증이 필요한 코드 패턴이 아님
8. **CI/CD 설정** — `.github/`, `Dockerfile`, `Procfile`, `.kamal/` 등은 인프라이며, 검증 스킬이 필요한 애플리케이션 패턴이 아님
