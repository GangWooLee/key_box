...
이 스킬이 순차 실행하는 검증 스킬 목록입니다. `/manage-rules`가 스킬을 생성/삭제할 때 이 목록을 자동 업데이트합니다.
...
검증 스킬이 없습니다. `/manage-rules`를 실행하여 프로젝트에 맞는 검증 스킬을 생성하세요.
```
...
| # | 스킬 | 파일 | 문제 | 수정 방법 |
|---|------|------|------|-----------|
| 1 | verify-<name1> | `app/models/user.rb:42` | 문제 설명 | 수정 코드 예시 |
| 2 | verify-<name2> | `app/controllers/users_controller.rb:15` | 문제 설명 | 수정 코드 예시 |
```
...
- [1/X] verify-<name1>: `app/models/user.rb` 수정 완료
- [2/X] verify-<name2>: `app/controllers/users_controller.rb` 수정 완료
...
| # | 스킬 | 파일 | 문제 |
|---|------|------|------|
| 1 | verify-<name> | `app/services/example_service.rb:42` | 자동 수정 불가 — 수동 확인 필요 |

수동으로 해결한 후 `/verify-rules`를 다시 실행하세요.
```
